...
import MultiSignatureViewV2 from '../components/sign/MultiSignatureViewV2';
...
import type { MultiSessionDetailsResponse, Stroke } from '../types';

type View = 'document' | 'sign';

const MultiSigningPage: React.FC = () => {
    const { sessionId } = useParams<{ sessionId: string }>();
    const navigate = useNavigate();
    const { t } = useTranslation();

    const [sessionDetails, setSessionDetails] = useState<MultiSessionDetailsResponse | null>(null);
    ...
    const loadSession = async () => {
        ...
        try {
            setLoading(true);
            const data = await signingApi.getMultiSession(sessionId);
    ...
    const handleSubmitAllSignatures = async (signatures: Map<string, Stroke[]>) => {
        if (!sessionId) return;
        ...
        try {
            ...
            const signaturesArray = Array.from(signatures.entries()).map(([documentSignerId, strokes]) => ({
                documentSignerId,
                signatureData: {
                    strokes,
            ...
            const response = await signingApi.submitMultiSignatures(sessionId, {
                signatures: signaturesArray,
                idempotencyKey,
            });

            if (response.success) {
                if (response.documentComplete) {
                    showToast.success(
                        t('signing.multi_sign_success', `All ${response.totalSignatures} signatures submitted successfully!`)
                    );
                    ...
                    showToast.warning(
                        t(
                            'signing.partial_success',
                            `${response.completedSignatures.length}/${response.totalSignatures} signatures submitted. Please retry remaining signatures.`
                        )
                    );
            ...
            handleSigningError(err, {
                ...
                onSigningInProgress: () => {
                    ...
                    setTimeout(() => handleSubmitAllSignatures(signatures), 1000);
                },
                onTooManyAttempts: () => {
                ...
                onDefault: (message) => {
                    showToast.error(message || t('errors.failed_to_sign', 'Failed to submit signatures'));
                },
            });
        } finally {
            setSubmitting(false);
                ...
                <div className="bg-white border-b border-secondary-200 px-4 py-2">
                    <div className="max-w-5xl mx-auto flex items-center justify-between">
                        <span className="text-sm text-secondary-600">
                            {t('signing.session_active', 'Session Active')} • {session.totalSignatures}{' '}
                            {t('signing.signatures_required', 'signatures required')}
                        </span>
            ...
            <div className="flex-grow flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8">
                <div
                    className={`w-full bg-white shadow-xl border border-secondary-200 flex flex-col rounded-xl overflow-hidden transition-all duration-300 ${
                        view === 'sign'
                            ? 'h-[calc(100vh-6rem)] sm:h-[90vh] max-w-6xl'
                        ...
                        <div className="flex flex-col h-full min-h-0 bg-white sm:bg-transparent">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs sm:text-sm text-secondary-500 mt-1">
                                            {t('signing.multi_sign_review', `Review document and sign ${session.totalSignatures} signature zones`)}
                                        </p>
                                    </div>
                                    <button onClick={handleBack} className="ml-4 text-secondary-600 hover:text-secondary-900">
                                        {t('common.back', 'Back')}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex-grow min-h-0 p-0 sm:p-4 bg-secondary-50">
                                <DocumentContentViewer
                                    ...
                                    signatureZones={pendingSignatures.map((s) => s.signatureZone)}
                                />
                            </div>
                            ...
                            <div className="flex-shrink-0 p-4 sm:pt-4 border-t border-secondary-200 sm:border-t-0 bg-white sm:bg-transparent z-10">
                                ...
                                >
                                    {t('signing.proceed_to_multi_sign', `Sign All (${session.totalSignatures})`)}
                                </button>
                            </div>
                        </div>
                    ) : (
                        // Multi-Signature View
                        <MultiSignatureViewV2
                            onBack={handleBack}
                            onSubmitAll={handleSubmitAllSignatures}
                            isSubmitting={submitting}
                            signatureZones={pendingSignatures}
                            documentTitle={document.title}
...
export default MultiSigningPage;
